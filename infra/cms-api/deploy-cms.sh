...
#
# 생성되는 리소스
#   S3   isuhani-cms-data            — CMS 포스트 + 이미지 + 소스 zip
...
DATA_BUCKET=${DATA_BUCKET:-isuhani-cms-data}
SITE_BUCKET=${SITE_BUCKET:-isuhani-test04}
CF_DISTRIBUTION_ID=${CF_DISTRIBUTION_ID:-EWBOXJOM7QWBG}
...
upload_source() {
  ...
  rm -f "$zip"
  cd "$ROOT"
  # admin UI를 사이트 빌드 산출물에 포함 — /admin/ 경로로 공개 배포
  mkdir -p frontend/public/admin
  cp cms/admin.html frontend/public/admin/index.html
  # frontend/ + buildspec.yml만 묶음 (node_modules 제외)
  cp "$BUILD_DIR/buildspec.yml" buildspec.yml
  # S3 마이그레이션 후 — 포스트는 S3가 단일 출처. 소스 zip에서 모두 제외.
  zip -rq "$zip" buildspec.yml \
    frontend/package.json frontend/package-lock.json frontend/next.config.js \
    frontend/tsconfig.json frontend/tailwind.config.ts frontend/postcss.config.js \
    frontend/next-env.d.ts \
    frontend/src frontend/public \
    -x 'frontend/node_modules/*' 'frontend/.next/*' 'frontend/out/*' \
       'frontend/public/uploads/cms/*' \
       'frontend/src/data/blog/embeddings.json' \
       'frontend/src/data/blog/posts/*' \
       'frontend/src/data/blog/index.json'
  rm -f buildspec.yml
...
build_lambda_zip() {
  ...
  zip -rq "$WORK/lambda.zip" index.mjs node_modules package.json
...
ensure_lambda_env() {
  ...
  "${AWS[@]}" lambda update-function-configuration \
    --function-name "$LAMBDA_NAME" \
    --environment "Variables={BUCKET=$DATA_BUCKET,BUILD_PROJECT=$BUILD_PROJECT,CMS_USER=$CMS_USER,CMS_PASSWORD=$pw}" \
    >/dev/null
  "${AWS[@]}" lambda wait function-updated --function-name "$LAMBDA_NAME"
}
